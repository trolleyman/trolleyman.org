...
fn main() {
	let mut app =
		App::new("trolleyman-org-xtask")
			.version("0.1.0")
			.about("Build runner for the trolleyman-org project")
			.author("Callum Tolley")
			.subcommand(SubCommand::with_name("build").about("Compile the project").arg(
				Arg::with_name("release").long("release").help("Build artifacts in release mode, with optimizations"),
			))
			.subcommand(SubCommand::with_name("run").about("Run the server locally").arg(
				Arg::with_name("release").long("release").help("Build artifacts in release mode, with optimizations"),
			))
			.subcommand(
